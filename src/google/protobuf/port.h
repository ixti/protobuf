...
class DebugCounter {
 public:
  ...
  DebugCounter(absl::string_view category, absl::string_view name) {
    // We want sorted output, so keep them in a std::map.
    CounterMap()[category][name] = this;
  }
  ...
  constexpr DebugCounter(absl::string_view, absl::string_view) {}
